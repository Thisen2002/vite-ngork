...
const OrgMngWidget: React.FC = () => {
  ...
  const fetchOrganizers = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_MAIN_API_URL || '/api'}/organizers`);
  ...
  const handleDelete = async (id: number | string | undefined) => {
    ...
    if (window.confirm('Are you sure you want to delete this organizer?')) {
      try {
        const response = await fetch(`${import.meta.env.VITE_MAIN_API_URL || '/api'}/organizers/${numericId}`, {
          method: 'DELETE',
        });
  ...
  const handleSearch = async () => {
    if (searchId !== null && searchId >= 1) {
      try {
        const response = await fetch(`${import.meta.env.VITE_MAIN_API_URL || '/api'}/organizers/${searchId}`);
        if (!response.ok) {
  ...
  const handleEditClick = async (organizer: any) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_MAIN_API_URL || '/api'}/organizers/${id}`);
      const data = await response.json();
